...
private:
     ...
     */
    void delete_tree(Node<K, V> *n) {	// this is from bst exercises
        if (n != nullptr){
        	delete_tree(n->left);
			delete_tree(n->right);
			delete n;
    ...
    int diameter(Node<K, V> *node) const {
        ...
        int h = 2 + height(node->left) + height(node->right);
        return std::max(h, std::max(diameter(node->left), diameter(node->right)));
    }
    ...
    size_t width(Node<K, V> *node, size_t level) const {
    	...
    	return width(node->left, level-1) + width(node->right, level-1);	//has both children
    }
    ...
    size_t sum_levels(Node<K, V> *node, size_t level) const {
    	if (node == nullptr){										// for some reason, the tree is null
			return 0;
		}
